...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating IS Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-is.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating CS Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-cs.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    infoln "Current working directory: $(pwd)"
    infoln "Executing cryptogen: cryptogen generate --config=./organizations/cryptogen/crypto-config-orderer.yaml --output=\"organizations\""
    set -x
    ...
    infoln "Listing contents of peer0.is.example.com/msp/signcerts after cryptogen:"
    ls -l organizations/peerOrganizations/is.example.com/peers/peer0.is.example.com/msp/signcerts
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    ${CONTAINER_CLI_COMPOSE} -f compose/compose-ca.yaml up -d 2>&1

    . organizations/fabric-ca/registerEnroll.sh

    # Wait for CA servers to be up and running
    infoln "Waiting for CA servers to start..."
    for CA_NAME in is cs ordererOrg; do
      while [ ! -f "organizations/fabric-ca/${CA_NAME}/tls-cert.pem" ]; do
        sleep 1
      done
      successln "${CA_NAME} CA is up."
    done
...
function networkUp() {
  ...
  # Create the postgres_data volume if it doesn't exist. This is idempotent.
  if [ "${POSTGRES}" == "true" ]; then
    infoln "Ensuring postgres_data volume exists"
    ${CONTAINER_CLI} volume create postgres_data > /dev/null 2>&1
  fi
  ...
  # Create the postgres_data volume if it doesn't exist
  if [ "${POSTGRES}" == "true" ]; then
    if ! ${CONTAINER_CLI} volume ls -q | grep -q "^postgres_data$"; then
      infoln "Creating postgres_data volume"
      ${CONTAINER_CLI} volume create postgres_data
    fi
  fi
  ...
  if [ "${POSTGRES}" == "true" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f compose/compose-postgres.yaml"
  fi

  DOCKER_SOCK="${DOCKER_SOCK}" ${CONTAINER_CLI_COMPOSE} -p net ${COMPOSE_FILES} up -d 2>&1
...
function networkDown() {
  COMPOSE_BASE_FILES="-f compose/compose-test-net.yaml -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-compose-test-net.yaml"
  COMPOSE_BFT_FILES="-f compose/compose-bft-test-net.yaml -f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-compose-bft-test-net.yaml"
  ...
  COMPOSE_POSTGRES_FILES="-f compose/compose-postgres.yaml"
  COMPOSE_FILES="${COMPOSE_BASE_FILES} ${COMPOSE_BFT_FILES} ${COMPOSE_COUCH_FILES} ${COMPOSE_CA_FILES} ${COMPOSE_POSTGRES_FILES}"
  ...
  if [ "${CONTAINER_CLI}" == "docker" ]; then
    DOCKER_SOCK=$DOCKER_SOCK ${CONTAINER_CLI_COMPOSE} -p net ${COMPOSE_FILES} ${COMPOSE_ORG3_FILES} down --volumes --remove-orphans
  elif [ "${CONTAINER_CLI}" == "podman" ]; then
    ${CONTAINER_CLI_COMPOSE} -p net ${COMPOSE_FILES} ${COMPOSE_ORG3_FILES} down --volumes
  else
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/is/msp organizations/fabric-ca/is/tls-cert.pem organizations/fabric-ca/is/ca-cert.pem organizations/fabric-ca/is/IssuerPublicKey organizations/fabric-ca/is/IssuerRevocationPublicKey organizations/fabric-ca/is/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/cs/msp organizations/fabric-ca/cs/tls-cert.pem organizations/fabric-ca/cs/ca-cert.pem organizations/fabric-ca/cs/IssuerPublicKey organizations/fabric-ca/cs/IssuerRevocationPublicKey organizations/fabric-ca/cs/fabric-ca-server.db'
