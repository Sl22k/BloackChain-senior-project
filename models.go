...
type DocumentVersion struct {
	...
	ChangeType       string              `json:"ChangeType"`       // "content", "approvers", "ownership", "decisions"
...
// NewVersionState represents the complete desired state for a new version
type NewVersionState struct {
	ContentHash      string   `json:"contentHash,omitempty"`    // New content hash (if file uploaded)
	ApproversList    []string `json:"approversList"`           // Complete new approvers list
	ValidDecisions   []string `json:"validDecisions"`          // Complete new valid decisions list
	ResetApprovals   bool     `json:"resetApprovals"`          // User choice (default: false)
