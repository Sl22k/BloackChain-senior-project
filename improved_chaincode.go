...
func (s *SmartContract) syncCurrentApprovalsToLatestVersion(doc *Document) {
	...
	if doc.Workflow.Enabled && len(doc.Workflow.Stages) > 0 {
		...
		// Determine if this is a simple (single stage) or advanced (multi-stage) workflow
		isSimpleWorkflow := len(doc.Workflow.Stages) == 1
		
		// Add approvals from ALL stages using appropriate key format
		for _, stage := range doc.Workflow.Stages {
			if stage.StageApprovals != nil {
				for approver, decision := range stage.StageApprovals {
					var key string
					if isSimpleWorkflow {
						// Simple workflow - use plain approver names (no stage prefix)
						key = approver
					} else {
						// Advanced workflow - use composite keys to distinguish stages  
						key = fmt.Sprintf("#%d %s", stage.StageNumber, approver)
					}
					doc.Versions[latestVersionIndex].ApprovalsMap[key] = decision
				}
			}
		}
	} else {
...
func (s *SmartContract) SubmitDocument(ctx contractapi.TransactionContextInterface, id, title, description, hash, uploader, validDecisionsJSON, approversJSON, workflowJSON, deadlineHours, stageDeadlineHours string) error {
	...
	if workflowJSON == "" {
		// Simple workflow - add approvers from the single stage with plain keys
		for approver, decision := range workflowConfig.Stages[0].StageApprovals {
			approvalsMap[approver] = decision
		}
	} else {
		// Advanced workflow - add approvers from ALL stages with composite keys
		for _, stage := range workflowConfig.Stages {
			for approver, decision := range stage.StageApprovals {
				key := fmt.Sprintf("#%d %s", stage.StageNumber, approver)
	...
	document := Document{
		...
		ApprovalsMap:      func() map[string]Decision {
			// Document-level ApprovalsMap shows only current active stage approvers
			currentStageApprovals := make(map[string]Decision)
			if len(workflowConfig.Stages) > 0 {
				currentStage := workflowConfig.Stages[workflowConfig.CurrentStage-1]
				for approver, decision := range currentStage.StageApprovals {
					var key string
					if workflowJSON == "" {
						key = approver
					} else {
						key = fmt.Sprintf("#%d %s", currentStage.StageNumber, approver)
					}
					currentStageApprovals[key] = decision
				}
			}
			return currentStageApprovals
		}(),
		ValidDecisions:    validDecisions,
