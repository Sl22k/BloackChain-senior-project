...
infoln "Current config.json:"
cat ${TEST_NETWORK_HOME}/channel-artifacts/config.json

# Modify the configuration to append the new org
set -x
echo "--- config.json before jq ---"
cat ${TEST_NETWORK_HOME}/channel-artifacts/config.json
echo "--- org4.json before jq ---"
cat ${TEST_NETWORK_HOME}/organizations/peerOrganizations/org4.example.com/org4.json
...
echo "--- modified_config.json after jq ---"
cat ${TEST_NETWORK_HOME}/channel-artifacts/modified_config.json
{ set +x; } 2>/dev/null
...
infoln "Config update generated:"
configtxlator proto_decode --input ${TEST_NETWORK_HOME}/channel-artifacts/org4_update_in_envelope.pb --type common.Envelope | jq .
